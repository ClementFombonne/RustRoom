...
impl Default for ImageViewer {
    fn default() -> Self {
        Self {
            ...
            // 800x600 is now just a temporary fallback for the fraction of a second
            // it takes the image loader to decode the true dimensions.
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            if let Some(tex) = dynamic_texture {
                ...
                if let Ok(egui::load::TexturePoll::Ready { texture }) =
                    egui_image.load_for_size(ctx, viewport_rect.size())
                {
